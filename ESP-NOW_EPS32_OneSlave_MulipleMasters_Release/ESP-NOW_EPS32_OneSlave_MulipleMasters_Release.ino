...
// CallBack用ブロードキャストMACアドレス  ff:ff:ff:ff:ff:ff
uint8_t gAddress[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};

const uint boardID = 0; // 端末を識別するボードID
const uint productID = 202008;
const uint maxID = 5;
const uint maxMoni = 4; // モニタ入力の最大値
uint selfMonitor = 0;
uint ignoreNum = 1;
const uint maketime = 250; // makeタイム
int dPin[32];              // digitalPin 初期化用
...
void OnDataRecv(const uint8_t *mac_addr, const uint8_t *incomingData, int len)
{
  ...
  Serial.printf("Self Monitor Value: %d \n", boardsStruct[myData.id - 1].moni);
  Serial.println();
  ...
  // 送信元の照合
  if (myData.pid == productID && myData.id != boardID && myData.moni <= maxMoni)
  {
    ...
    cbLED(bLED, 1, 20); // LED No, 回数, Delay量
      ...
      if (myData.moni != selfMonitor - 1)
      {
        monitor_select(myData.moni);
        cbLED(rLED, 1, 20); // LED No, 回数, Delay量
      }
      else
      {
      }
